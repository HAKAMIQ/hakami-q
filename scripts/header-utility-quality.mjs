...
requireMatch(headerComponent, /\.utility-links a\s*\{[\s\S]*?font-size\s*:\s*var\(--font-size-body-1\)/, 'Header utility links must remain Body 1 (14px).');
requireMatch(headerComponent, /\.utility-links a\s*\{[\s\S]*?font-weight\s*:\s*var\(--font-weight-semibold\)/, 'Header utility links must remain semibold.');
...
if (/\.utility-links a\s*\{[\s\S]*?font-size\s*:\s*var\(--font-size-caption-[12]\)/.test(compactDesktop)) {
...
requireMatch(css, /\.utility-links a\s*\{[\s\S]*?font-size\s*:\s*var\(--font-size-body-1\)/, 'Persistent desktop utility links must agree on Body 1.');
